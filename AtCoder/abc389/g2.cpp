...
void solve(){
	cin >> n >> p;
	int M = n*(n-1)/2;
	vector dp(n/2+1, vector(n/2+1, vector(M+1, vector(n/2+1, 0LL))));
	...
	for (int lado1 = 1; lado1 <= n/2; ++lado1) {
		for (int lado2 = 1; lado2 <= n/2; ++lado2) {
			for (int tot_edg = lado1+lado2-1; tot_edg <= (lado1+lado2)*(lado1+lado2-1)/2; ++tot_edg) {
				for (int last = 1; last <= lado2; ++last) {
					for (int eu = 1; eu <= lado1; ++eu) {
						auto& DP = dp[lado1][lado2][tot_edg][eu];											
						for (int edg = eu; edg <= min(tot_edg, eu*(eu-1)/2 + eu*last); ++edg) {
							int k = dp[lado2][lado1-eu][tot_edg-edg][last] * dp1[last][eu][edg] % p;
	...
	for (int i = n-1; i <= M; ++i) {
		int ans = 0;
		for (int j = 1; j <= n/2; ++j) add(ans, dp[n/2][n/2][i][j]);
		cout << ans << ' ';
	}
	cout << endl;
}
