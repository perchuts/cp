...
void solve(){
	...
	auto solve = [&] (vector<ii> v, int bound) {
		...
		sort(all(tipos));
		vector<pair<ii, int>> qnt;

		int last = 0, best = 0;
		for (int l = 0, r = 0; l < sz(tipos); l = r) {
			qnt.pb({sum, tipos[l].first - last});
		...
		sort(begin(qnt), end(qnt));
		for (int l = 0, r = 0; l < sz(qnt); l = r) {
			int tot = 0;
			while (r != sz(qnt) and qnt[l].first == qnt[r].first) tot += qnt[r++].second;
			ckmax(best, tot);
		}
		return best;
	};
	...
	int ansx = n, ansy = m;
	for (int i = 0; i < 10; ++i) ansx = min(ansx, solve(x, n)), ansy = min(ansy, solve(y, m));
	cout << ansx * ansy << endl;
}
