...
void solve(){
	...
	while (true) {
		...
		bool ok = 1;
		for (int u = 0; u < n; ++u) {
			if (vis[u] == 0 or sz(g[u]) == 0) continue;
			assert(deg[u] != 0);
			if (deg[u] % bint(sz(g[u])) != bint(0)) {
				for (int d = sz(g[u])-1; d >= 1; --d) {
					if (bint(sz(g[u]) % d) != bint(0) or deg[u] % bint(d) != bint(0)) continue;
					sexo *= bint(sz(g[u]) / d);
					break;
				}
				ok = 0;
