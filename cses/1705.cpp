...
void solve(){
	...
	while (q--) {
		int u, v, c; cin >> u >> v >> c;
		--u, --v, --c;
		// basicamente quero ver se c eh ponto de articulacao e c esta entre u e v
		if (u == c or v == c) {
			cout << "NO" << endl;
			continue;
		}
		...
		u = buceta.pos[u], v = buceta.pos[v], c = buceta.pos[c];
		if (u == v) {
			cout << "YES" << endl;
			continue;
		}
		...
		if (x == c or y == c) {
			if (buceta.lvl[z] <= buceta.lvl[c]) cout << "NO" << endl;
			else cout << "YES" << endl;
		} else cout << "YES" << endl;
	}
}
