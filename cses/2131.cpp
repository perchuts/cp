...
void solve(){
	...
	for (int i = 0; i < n; ++i) {
		fluxo.add(src, i, a[i], 0);
		fluxo.add(n+i, sink, b[i], 0);	
	}
	for (int i = 0; i < n; ++i) for (int j = 0; j < n; ++j) fluxo.add(i, n+j, 1, -grid[i][j]);
	auto [k, cost] = fluxo.min_cost_flow(src, sink);
	if (k != s1) {
		cout << -1 << endl;
		return;
	}
	...
	for (int i = 0; i < n; ++i) {
		for (auto edg : fluxo.g[i]) {
			if (n <= edg.to and edg.to < 2*n and edg.flow == 1) {
				ans[i][edg.to-n] = 'X';
			}
		}
	}
