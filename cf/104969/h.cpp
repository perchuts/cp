...
void solve(){
	int n, m; cin >> n >> m;
	vector<pt> borda(m), sabor(n);
	for (auto& [x,y] : sabor) cin >> x >> y;
	for (auto& [x, y] : borda) cin >> x >> y;
	borda.emplace_back(0, 0);
	convex_pol fodase(borda);
	int ans = 0;
	for (auto pp : sabor) ans += fodase.is_inside(pp); 
	cout << ans << endl;
