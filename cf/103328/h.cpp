...
void solve(){
	int n, m, k; cin >> n >> m >> k;
	vector<int> a(n), pref(m+1);
	for (auto& x : a) cin >> x, pref[x]++;
	vector<mint> fat(3001, 1), ifat(3001, 1), dp(n+1, 1);
	for (int i = 1; i <= 3000; ++i) fat[i] = i * fat[i-1], ifat[i] = mint(1) / fat[i];
	for (int i = 1; i <= m; ++i) {
		pref[i] += pref[i-1];
		vector<mint> ndp(n+1);
		for (int j = 0; j <= n; ++j) {
			ndp[j] = (dp[j] - (j <= k ? 0 : dp[j-k-1]) + mod) % mod;
		}
	}
	mint ans = m;
	ans ^= n;
