...
void solve(){
	...
	int ans = 1e18, lst_k = -1, cells = 0;
	dinitz fluxo(n+m+10);
	int src = n+m+5, sink = n+m+6;
	for (int i = 0; i < n; ++i) for (int j = 0; j < m; ++j) {
		if (grid[i][j] == '.') fluxo.add(i, n+j, 1), cells++;	
	}
	ans = cells * d;
	for (int j = 1; j <= max(n, m); ++j) {
		for (int i = 0; i < n; ++i) fluxo.add(src, i, 1);
		for (int i = 0; i < m; ++i) fluxo.add(n+i, sink, 1);
		int k = fluxo.max_flow(src, sink);
		if (k == lst_k) break;
		lst_k = k, ckmin(ans, d*(cells-k) + j*c);
	}
	cout << ans << endl;
}
